...
void Qsort(vector<string> &strs, int low, int high)
{
	if (low >= high)
	{
		return;
	...
	int first = low;
	int last = high;
	string key = strs[first];
	while (first < last)
	{
		while (first < last && strs[last].length() >= key.length())
			last--;
		strs[first] = strs[last];

		while (first < last && strs[first].length() <= key.length())
			first++;
		strs[last] = strs[first];
	}

	strs[first] = key;

	if (low < first - 1)
		Qsort(strs, low, first - 1);
	if (high > first + 1)
		Qsort(strs, first + 1, high);
}
...
string longestCommonPrefix(vector<string> &strs) 
{
	...
	{
		string tmp = CommonPrefix(strs[0], strs[i]);
		CP = (CP.length() < tmp.length()) ? (CP) : (tmp);
	}
...
vector<string> UnitTest(string A[], int length)
{
	vector<string> tmp;
...
{
	vector<string> strs;
	string tmp[4] = { "abab","aba","a", "afjie"};

	strs = UnitTest(tmp, 4);
	strs.clear();
	cout << "CP is " << longestCommonPrefix(strs) << endl;
