...
public:
	...
	{
		stack<int> stemp;
		PreSum(root, 0, 0, stemp);

		for (vector<vector<int> >::iterator i = asum.begin(); i != asum.end(); i++)
		{
	...
	void PreSum(TreeNode *temp, int sum, int tmp, stack<int> stemp);
...
void dump(stack<int> temp)
{
	int length = temp.size();
	for (size_t i = 0; i < length; i++)
	{
		Sum.push_back(temp.top());
		temp.pop();
	}
...
void Btree::PreSum(TreeNode *temp, int sum, int tmp, stack<int> stemp)
{
	if (temp != NULL)
	{
		...
		else
		{
			tmp += temp->val;
			stemp.push(temp->val);
			PreSum(temp->left, sum, tmp, stemp);
			PreSum(temp->right, sum, tmp, stemp);
		}
	}
}
