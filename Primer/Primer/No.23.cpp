...
bool isValid(string s) 
{
	...
	{
		if (Hecate.empty() && deferos.find(*i) != string::npos)
		{
			return false;
		}

		if (aspros.find(*i) != string::npos)
		{
			Hecate.push_back(*i);
		}

		if (deferos.find(*i) != string::npos)
		{
			if (key[Hecate.back()] != *i)
			{
				return false;
			}
			else
...
void UnitTest(string s)
{
	if (isValid(s))
	{
		cout << s << " is vaild" << endl;
	}
	else
	{
		cout << s << " is not vaild" << endl;
	}
}
