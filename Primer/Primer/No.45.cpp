...
//int greedy(int index, vector<int> &gas, vector<int> &cost)
//{
//	int length = gas.size();
//	int car = 0;
//
...
//			return -1;
//
//		car = gas[(i + index) % length] - cost[(i + index) % length];
//	}
//
//	if (car >= 0)
//		return index;
...
int greedy(int i, int j, vector<int> &way, int sum)
{
	if (i + 1 == j)
		return j;
	...
	{
		sum += way[i];
		return greedy(i + 1, j, way, sum);
	}
	else
		return greedy(i, j - 1, way, sum);
}
...
int canCompleteCircuit(vector<int> &gas, vector<int> &cost)
{
	...
	if (sum < 0)
		...
		return greedy(0, way.size() - 1, way, 0);

}

int main()
{
	int A[9] = { 2, 4, 6, 4, 8, 1, 2, 6, 1 };
