...
public:
	...
	void display4()
	{
		vector<vector<int>> test = levelOrderBottom(root);
		//int Max = 0;
		//MaxDepth(root, 0, Max);
		//cout << Max;

	}
	//void MaxDepth(tree *, int count, int &Max);
	int MaxDepth(tree *);
...
//void Btree::MaxDepth(tree *temp, int count, int &Max)
//{
//	if (temp != NULL)
//	{
...
//		}
//
//		MaxDepth(temp->left, count + 1, Max);
//		MaxDepth(temp->right, count + 1, Max);
//	}
//}
...
vector<vector<int>> Btree::levelOrderBottom(tree *root)
{
...
void Btree::getElement(vector<int> &level, int count, int len, tree *root)
{
	if (root != NULL)
	{
		if (count == len)
